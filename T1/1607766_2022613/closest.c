...
int main (int argc, char *argv[])
{
	...
	if ((f_inputs = fopen(argv[1], "r")) == NULL)
	{
			return -1;
	}
	...
	printf ("%f %f %f %f %f %f ", time_spent,dist, menor_a->x, menor_a->y, menor_b->x, menor_b->y);
	...
	//imprime o resultado na tela e no arquivo de saida
	printf ("%f %f %f %f %f %f\n", time_spent,dist, menor_a->x, menor_a->y, menor_b->x, menor_b->y);
	...
	{	
		...
		{	
			...
			if (tipo == 'x')
			{
				...
				{
					//usa o y como critério de desempate caso x seja igual
...
float divisao_e_conquista (Ponto* inputs, int size, Ponto** first, Ponto** second)
{
	...
	//-----------------------------------------------------
	//chamada o método em
    dist = find_closest (ord_x, ord_y, size, first, second);
...
{	
	...
	//-----------------------------------------------------
	//chamadas recursivas
	d_l = find_closest (ord_x, ord_y_l, size_l, &left_a, &left_b);
