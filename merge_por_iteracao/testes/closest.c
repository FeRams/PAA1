...
int main ()
{
	...
	while (qtdPontosAtual <= size)
	{
		printf("Qtd pontos atual %d \n", qtdPontosAtual);
		...
		clock_t begin = clock();
		dist = sqrt(forca_bruta (inputs, qtdPontosAtual, &menor_a, &menor_b));
		clock_t end = clock();
		double time_spent_brute = (double)(end - begin)/1000;	
		/**********************************************/
		//imprime o resultado na tela e no arquivo de saida
		printf ("\n ---------------------------------\n");	
		printf ("FORCA BRUTA\n");
		printf("menor distancia : %f, entre (%f, %f), e (%f, %f)\n tempo: %lfms",
		 dist, menor_a->x, menor_a->y, menor_b->x, menor_b->y, time_spent_brute);	
		printf ("\n ---------------------------------\n");
		/**********************************************/
		//mede o tempo de execucao do divisao e conquista em ms
		begin = clock();
		dist = divisao_e_conquista (inputs, qtdPontosAtual, &menor_a, &menor_b);
		end = clock();
		double time_spent_div = (double)(end - begin)/1000;	
		/**********************************************/
		//imprime o resultado na tela e no arquivo de saida
		printf ("\n ---------------------------------\n");	
		printf ("DIVISAO E CONQUISTA\n");
		printf("menor distancia : %f, entre (%f, %f), e (%f, %f)\n tempo: %lfms",
		 dist, menor_a->x, menor_a->y, menor_b->x, menor_b->y, time_spent_div);	
		printf ("\n ---------------------------------\n");	
		/**********************************************/
		//imprime o resultado na tela e no arquivo de saida
		fprintf(f_outputs, "%d %lf %lf \n", qtdPontosAtual, time_spent_brute, time_spent_div);
		/**********************************************/

		if (qtdPontosAtual == size)
			break;

		if (qtdPontosAtual % (incremento*10) == 0)
		{
			incremento *= 10;
		}
		qtdPontosAtual += incremento;
	}

	printf("Qtd pontos atual FIM%d \n", qtdPontosAtual);
	fclose (f_inputs);
	free (inputs);
...
float divisao_e_conquista (Ponto* inputs, int size, Ponto** first, Ponto** second)
{
	...
	//-----------------------------------------------------
	//chamada recursiva
    dist = recursivo (ord_x, size, first, second);
...
float recursivo (Ponto** ord_x, int size, Ponto** first, Ponto** second)
{	
	//-----------------------------------------------------
	...
	//-----------------------------------------------------
	//chamadas recursivas
	d_l = recursivo (ord_x, size_l, &left_a, &left_b);
	d_r = recursivo ((&ord_x[size_l]), size_r, &right_a, &right_b);
	//-----------------------------------------------------
	//**************************************************************************
	//CONQUISTA
	...
	//inicializa os vetores
	Ponto** ord_next = (Ponto**) malloc (size * sizeof(Ponto*));
	...
	Ponto** ord_y_l = ord_next;
	Ponto** ord_y_r = &(ord_next[size_l]);
	merge_sort (ord_y_l, 0, size_l-1, "y");
	merge_sort (ord_y_r, 0, size_r-1, "y");
